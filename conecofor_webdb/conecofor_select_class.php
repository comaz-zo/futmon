...
class conecofor_select_class {
	
	var $conn_str;
	var $dbh;
	
    function conecofor_select_class(){
    ...
    function print_title(){
        ...
        switch ($_SESSION['conecofor_select_status']){
    		case 'SELECT_AREA';
    			//$html_string = $html_string . 'Torna al menu principale &nbsp;';
                $html_string = $html_string . '<a href="conecofor_menu.php"><img src="images/previous.png" border="0" style="width: 20px; height: 20px;"></a>&nbsp;';
		        break;
			default;
				...
				//$html_string = $html_string . 'Torna alla lista aree inserite &nbsp;';
                $html_string = $html_string .  '<INPUT type="image" src="images/previous.png" name="conecofor_next_image" style="width: 20px; height: 20px;"/>';
    ...
    function select_area(){
		...
		try
		{
		    ...
		    $sql_query = sprintf("SELECT * FROM ".DB_TABLE_CONECOFOR_AREE." WHERE ( ANNO_RIL = ".$anno_rilievi." AND IDSQUADRA = '".$_SESSION['conecofor_login_idutente']."' ) ORDER BY IDAREA");
		    $odd = 1;
            foreach ($this->dbh->query($sql_query, PDO::FETCH_ASSOC) as $row)
		    {
                if($odd%2){
    ...
    function create_pdfreport($id_area){
	    ...
	    $pdf->SetFont('','B',12);
	    $pdf->Cell(35);
	    $pdf->Cell(120,10,'CONECOFOR 2012 AREA: '.$id_area,1,0,'C');
	    $pdf->SetFont('','',8);
    ...
    function execute_delete_area($verbose=true){
	    ...
	    $sql_query = sprintf("DELETE FROM %s WHERE IDAREA ='%s'", $anno_rilievi."_".DB_TABLE_CONECOFOR_AREE,$_SESSION["conecofor_select_idarea"]);
	    ...
	    $sql_query = sprintf("DELETE FROM %s WHERE IDAREA ='%s'", $anno_rilievi."_".DB_TABLE_CONECOFOR_PIANTE,$_SESSION["conecofor_select_idarea"]);   
		$sth = $this->dbh->prepare($sql_query);
   		try{
	   		$sth->execute ();
	   		if($verbose){
	    ...
	    $sql_query = sprintf("DELETE FROM %s WHERE IDAREA ='%s'", $anno_rilievi."_".DB_TABLE_CONECOFOR_SINTOMI,$_SESSION["conecofor_select_idarea"]);   
		$sth = $this->dbh->prepare($sql_query);
   		try{
	   		$sth->execute ();
	   		if($verbose){
	    ...
	    $sql_query = sprintf("DELETE FROM %s WHERE IDAREA ='%s'", $anno_rilievi."_".DB_TABLE_CONECOFOR_CAV,$_SESSION["conecofor_select_idarea"]);   
		$sth = $this->dbh->prepare($sql_query);
   		try{
	   		$sth->execute ();
	   		if($verbose){
